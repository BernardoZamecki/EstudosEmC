...
representando respectivamente o número de refeições disponiveis de frango, bife
e massa. A segunda linha contem três inteiros Cr, Br e Pr (0 ≤ Cr, Br, Pr ≤ 100),
indicando respectivamente o número de refeições requisitadas de frango, bife e
...
int main()
{
    int numF, numB, numM, pedF, pedB, pedM, faltas = 0;
    scanf("%d %d %d %d %d %d", &numF, &numB, &numM, &pedF, &pedB, &pedM);
    if (numF < pedF)
    {
        faltas = faltas + (pedF - numF);
    }
    if (numB < pedB)
    {
        faltas = faltas + (pedB - numB);
    }
    if (numM < pedM)
    {
        faltas = faltas + (pedM - numM);
    }
    printf("%d\n", faltas);
